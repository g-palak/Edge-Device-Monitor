...
WebSockets provide bidirectional, persistent connections — critical for edge systems where the cloud needs to push configs to devices without polling. The backend maintains separate channels: frontend clients receive all telemetry broadcasts, while each device gets its own channel and only receives configs addressed to it. This mirrors the security boundary between monitoring dashboards and device command-and-control.
